...
//  private val SLICE_SIZE      = (OSC_BUF_SIZE - 27) / 5 // = 1633 or roughly 3 ms at 48 kHz; 4 bytes per float, 1 byte per type-tag : < 64K
  private val SLICE_SIZE_2    = (OSC_BUF_SIZE - 37) / 5 // = 1631, when using two ranges
  ...
  object Config {
    ...
    def default(server: Server, bufId: Int, useFrames: Int, numChannels: Int, nodeId: Int): Config = {
      val SLICE_SIZE_2M = SLICE_SIZE_2 - 1
      val useSize       = useFrames * numChannels
      val slicesPerUse  = (useSize + SLICE_SIZE_2M) / SLICE_SIZE_2
      ...
      val numSlices     = (size0 * numChannels + SLICE_SIZE_2M) / SLICE_SIZE_2
      val bufSize       = numSlices * SLICE_SIZE_2
...
//    require (bufSize % SLICE_SIZE == 0)
//
//    val numSlices : Int = if (isEmpty) 0 else bufSize / SLICE_SIZE
  }
}

class JScopeView2 extends JComponent with ScopeViewLike {
  import JScopeView2._
  ...
  */
  ...
  private class State(val c: Config) extends PartialFunction[Message, Unit] { self =>
...
//    sliceIdx = -1
//    config.server ! NodeSet(config.nodeId, "res" -> 1)
//  }
...
//    case Trigger(id, trigId, trigVal) =>
//      if (id == config.nodeId) {
//        ???
//      }
//
